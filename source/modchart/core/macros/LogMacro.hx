...
class LogMacro {
	...
	public static function init() {
		...
		Compiler.addGlobalMetadata("modchart", "@:build(modchart.core.macros.LogMacro.build())");

		Context.info('CALL DEBUGGER ENABLED', Context.currentPos());
		#end
	}
			...
			switch (field.kind) {
				case FFun(func):
					...
					if(hasMeta(field.meta, ":to")) {
						...
						shouldAddPos = false;
					} else if (field.name == "keyValueIterator" || field.name == "update" || field.name == "draw" || field.name == "destroy") {
						shouldAddPos = false;
